...
.. math:: 
    \begin{equation}
    \begin{aligned}
    資訊損失量 &= -\log_{2}{(概化處理前與概化處理後資料之數值的出現次數比值)}\\ &=  -\log_{2} {(\frac{概化處理前資料之數值於資料行的出現次數}{概化處理後資料之數值於資料行的出現次數})}
    \end{aligned}
    \end{equation}
...
假設一筆 **獨特資料** ，經過概化處理後變得 **常見** ，表示「概化處理前資料之數值於資料行的出現次數」較低，「概化處理後資料之數值於資料行的出現次數」較高，因此「出現次數比值」較小，資訊損失量較大。

假設一筆 **獨特資料** ，經過概化處理後依然 **獨特** ，表示「概化處理前資料之數值於資料行的出現次數」較低，「概化處理後資料之數值於資料行的出現次數」較低，因此「出現次數比值」較高，資訊損失量較小。

同理， **常見資料** 經過概化處理後變得 **常見** 亦然，表示資訊損失量較小。


以 **編號1** 儲存格為例，數值 1970 在「原始資料表」中出現 1 次，經過概化處理後變成 197* ，在「去識別化資料表」中出現 1 次，故：
...
以 **編號2** 儲存格為例，數值 1981 在「原始資料表」中出現 1 次，經過概化處理後變成 198*，在「去識別化資料表」中出現 3 次，故：
...
同理，**編號3** 與 **編號4** 儲存格之資訊損失量皆為 1 。



**STEP2: 計算資料表整體的資訊損失量。**

再來，加總所有儲存格損失資訊量，得到整體資訊損失量： 
...
資料表的可損失之最小資訊量，爲所有儲存格可損失之最小資訊量的加總。而當儲存格具有可損失之最小資訊量，即代表儲存格之數值，經概化處理後沒有變化，「出現次數比值」為 1 。因此，資料表的可損失之最小資訊量可如下計算：
...
資料表的可損失之最大資訊量，爲所有儲存格可損失之最大資訊量的加總。而當儲存格具有可損失之最大資訊量，即代表儲存格之數值，原只出現 1 次 ，經概化處理後，與整個資料行之數值相同。
...
.. math:: 
    ...
    \\ &= 資料表的儲存格數量 \times -\log_{2} {(\frac{概化處理前資料之數值於資料行的出現次數}{概化處理後資料之數值於資料行的出現次數})}

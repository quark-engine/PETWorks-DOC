...
以下程式碼旨在對資料進行去識別化以滿足 :math:`l`-多樣性（ :math:`l`-diversity）。更多 :math:`l`-多樣性之說明，詳見 `此處 <#id4>`_ 。
...
本專案之 :math:`l`-多樣性去識別化 API，為串接開源工具 ARX 之去識別化演算法進行。該演算法的處理流程可簡述如下：

1. 參考個體背景資訊定義與屬性型態定義，列舉並排序所有可能的去識別化處理強度組合。
2. 利用二分搜尋法，找出滿足 :math:`l` 值設定，且去識別化處理強度最弱的組合。
